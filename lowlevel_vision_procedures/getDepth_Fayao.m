...
varargin={};
run( [depth_homedir '/depth-fayao/libs/vlfeat-0.9.18/toolbox/vl_setup']);
dir_matConvNet= [depth_homedir '/depth-fayao/libs/matconvnet_20141015/matlab/'];
addpath(genpath(dir_matConvNet));
addpath([ depth_homedir '/depth-fayao/demo']);
run([dir_matConvNet 'vl_setupnn.m']);

opts=[];

% % some indoor image examples:
img_dir= [depth_homedir '/depth-fayao/custom_indoor_sample/']; 
img_type='indoor';
...
result_dir=fullfile([ depth_homedir '/depth-fayao'], 'results', img_dir_name);
...
if strcmp(img_type, 'outdoor') 
    ...
    %outdoor scene model
    trained_model_file=[ depth_homedir '/depth-fayao/model_trained/model_dcnf-fcsp_Make3D']; 
end
...
if strcmp(img_type, 'indoor')
    ...
    %indoor scene model
    trained_model_file=[ depth_homedir '/depth-fayao/model_trained/model_dcnf-fcsp_NYUD2'];   
end
...
%    img_data=read_img_rgb(full_img_file);
    ...
    depths_superpxl = do_model_evaluate(model_trained, ds_info, opts_eval);
...
%     fprintf('inpaiting using Anat Levin`s colorization code, this may take a while...\n');
    depths_pxl = do_inpainting(depths_superpxl, img_data, sp_info);
% 
