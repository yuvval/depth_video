...
for frameId = frames_range
    imageName = fullfile(base_path,sprintf('/EvaluationSet/%s/rgb/r-%d-%d.png', video_name, frames.imageTimestamp(frameId), frames.imageFrameID(frameId)));
    im = imread(imageName);
    if cnt == 1
        imsize = size(im);         
    ...
    depthName = fullfile(base_path,sprintf('/EvaluationSet/%s/depth/d-%d-%d.png', video_name, frames.depthTimestamp(frameId), frames.depthFrameID(frameId)));
    depth_frames(:,:, cnt) = imread(depthName);
    depth_frames(:,:, cnt) = bitor(bitshift(depth_frames(:,:, cnt),-3), bitshift(depth_frames(:,:, cnt),16-3));
    depth_frames(:,:, cnt) = double(depth_frames(:,:, cnt));
