function [depth_frames, depth_sp, sp_frames, num_sps] = calc_depth(video, prepr_params)
%
%
...
[depth_sp, sp_frames] = deal( cell(num_frames,1));


switch prepr_params.depth.depth_method
  case 'fayao'
    
    %% TBD fix cond load + relate to super pixels
    % depth_frames = nan(size(video,1), size(video,2), num_frames);
    % for t = 1:num_frames
    %     [depth_sp{t}, depth_frames(:,:,t), sp_frames{t}] = ...
  ...
  case 'eigen'
    ...
    % For each frame, assign MEDIAN depth to each superpxl.
    for t = 1:num_frames
        ...
        for k = 1:num_sps{t} % NOT EFFICIENT TODO - IMPLEMENT FASTER
            depth_sp{t}(k) = median(depth_frame(sp_frame==k));
        end
